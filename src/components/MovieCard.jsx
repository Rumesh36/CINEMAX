import React, { useState } from "react";
import { motion } from "framer-motion";

const MovieCard = ({ img, title, director, release, plot }) => {
  const [flipped, setFlipped] = useState(false);

  const handleSwipe = (_, info) => {
    const swipe = info.offset.x;

    if (Math.abs(swipe) > 100) {
      setFlipped((prev) => !prev);
    }
  };

  return (
    <div>
      <div className="bg-white border-black w-[350px] h-[650px] flex flex-col items-center rounded-xl">
        
        {/* Swipeable image area */}
        <div className="mt-8 w-[300px] h-[450px] cursor-grab [perspective:1000px]">
          <motion.div
            className="relative w-full h-full transition-transform duration-700"
            drag="x"
            dragConstraints={{ left: 0, right: 0 }}
            onDragEnd={handleSwipe}
            animate={{ rotateY: flipped ? 180 : 0 }}
            style={{ transformStyle: "preserve-3d" }}
          >
            {/* Front Image */}
            <div className="absolute w-full h-full rounded-2xl backface-hidden">
              <img
                className="w-full h-full object-cover rounded-2xl"
                src={img}
                alt={title}
              />
            </div>

            {/* Back Plot */}
            <div className="absolute w-full h-full bg-gray-800 text-white font-medium text-md rounded-2xl flex items-center justify-center p-4 backface-hidden [transform:rotateY(180deg)]">
              <p className="text-center whitespace-pre-line leading-relaxed px-2">{plot}</p>
            </div>
          </motion.div>
        </div>

        {/* Static Details */}
        <div className="mt-3 flex flex-wrap gap-3 justify-center w-70">
          <h1 className="text-xl font-bold bg-gray-900 text-white px-3 py-2 rounded-lg">{title}</h1>
          <h1 className="text-xl font-bold bg-gray-200 border-black border-2 px-4 py-2 rounded-lg">{release}</h1>
          <h1 className="text-xl font-bold border-2 text-red-600 px-5 py-2 rounded-lg">{director}</h1>
        </div>

        <button className="text-gray-700 font-medium text-lg mt-2">Swipe image to view plot</button>
      </div>
    </div>
  );
};

export default MovieCard;
